...
router.get('/available/:bookId', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
...
router.post('/issue', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await db.run(
      'INSERT INTO bookIssues (bookId, userId, issueDate, returnDate, remarks, status) VALUES (?, ?, ?, ?, ?, ?)',
      [bookId, userId, issueDate, returnDate, remarks || '', 'issued']
    );
...
router.get('/user/:userId', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const issues = await db.all(
      `SELECT bi.*, b.title, b.author, b.serialNo 
       FROM bookIssues bi 
...
// Return book
router.post('/return', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    ...
    await db.run(
      'UPDATE bookIssues SET status = ? WHERE id = ?',
      ['returned', issueId]
    );

    const fineResult = await db.run(
      'INSERT INTO fines (issueId, actualReturnDate, fineAmount, isPaid) VALUES (?, ?, ?, ?)',
      [issueId, actualReturnDate, fineAmount, fineAmount === 0 ? 1 : 0]
    );
...
// Get fine details
router.get('/fine/:fineId', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const db = await getDatabase();
...
// Pay fine
router.put('/fine/:fineId', authenticateToken, async (req: AuthRequest, res: Response) => {
  try {
    const { remarks } = req.body;
    const db = await getDatabase();

    await db.run(
      'UPDATE fines SET isPaid = ?, remarks = ? WHERE id = ?',
      [1, remarks || '', req.params.fineId]
    );
